...
void solve(){
    ...
    forn(i,n){
        ...
        //b[i] -> se refiere a los caminos entre i-1 e i
    }
    //n alto m ancho
    //meter en vertical el camino tomado por el corredor m_i
    vvi ans(n,vector<int>(m,-1));
    int mc = m,corredor = 0;
    while(mc--){
        int x,i,j;tie(x,i,j) = pq.top();
        pq.pop();
        //vemos si jala con i
        ans[i][corredor]=-1 * x;
        corredor++;
    ...
    dbg(ans);
    //ya tenemos la mejor respuesta, ahora solo es llenar con los peores que se pueda, el resto del camino de cada corredor
    forn(i,n){
        forn(j,m){
            if(ans[i][j]==-1){
                auto xd = b[i].rbegin();
                ans[i][j] = *xd;
                b[i].erase(prev(b[i].end()));
            }
        }
    }
