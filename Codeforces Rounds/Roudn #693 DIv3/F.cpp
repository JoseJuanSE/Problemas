...
void solve(){
    ...
    for(int i=1;i<a1.size();i++){
        if((a1[i]-a1[i-1])%2==0)a=true;
    }
    for(int i=1;i<b2.size();i++){
        if((b2[i]-b2[i-1])%2==0)a=true;
    }
    for(int i=1;i<min(SZ(a1),SZ(b2));i++){
        if(abs(a1[i]-b2[i-1])%2==1)a=true;
        if(abs(a1[i-1]-b2[i])%2==1)a=true;
        if(abs(a1[i]-b2[i])%2==1)a=true;
    }
    if(a)cout<<"NO"<<endl;
    else cout<<"YES"<<endl;
}
