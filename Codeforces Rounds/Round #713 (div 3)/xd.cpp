...
void calc(){
    long long maxi = 0;
    vector<long long> nd(3000005);
    for(long long i=2;i<3000001;i++)
        nd[i] = i+1;
    nd[1] = 1;
    for(long long i=2;i<3000001;i++){
        for(long long j=2*i;j<3000001;j+=i){
            nd[j]+=i;
            if(nd[j] == 2712823)cout<<"------------"<<j<<endl;
        }
    }
    forr(i,1,3000001){
        if(ans.find(nd[i])==ans.end())ans[nd[i]] = i;
        if(nd[i] == 2712823) cout<<"hola\n";
    }
}
void solve(){
    ll n;
    cin>>n;
    if(ans.find(n)==ans.end())cout<<-1<<endl;
